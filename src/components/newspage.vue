<template>
  ...
  const handleCommentSubmit = async () => {
    ...
    try {
      const response = await axios.post('https://view-truth.zeabur.app/api/news/comment/create', {
  ...
  const CommentTree = defineComponent({
    ...
    setup(props, { emit }) {
      ...
      const submitReply = async (comment: any) => {
        ...
        try {
          await axios.post('https://view-truth.zeabur.app/api/news/comment/create', {
            uid: newsStore.userState.user?.uid,
            una: newsStore.userState.user?.una,
            nid: route.params.id,
